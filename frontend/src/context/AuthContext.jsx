...
export const AuthProvider = ({ children }) => {
  ...
  useEffect(() => {
    const cachedUser = DatabaseHelper.getUserData();
      ...
      apiClient.get('/api/auth/profile')
        .then((res) => {
          if (res.data && res.data.success === true) {
            ...
            DatabaseHelper.saveUserData(res.data.user);
          }
        })
        .catch(() => {
  ...
  const login = async (email, password) => {
    try {
      ...
      if (response.data && response.data.success === true) {
        ...
        DatabaseHelper.saveTokens(tokens.accessToken, tokens.refreshToken);
        DatabaseHelper.saveUserData(userData);
  ...
  const updateProfile = async ({ name }) => {
    try {
      ...
      if (response.data && response.data.success === true) {
        ...
        DatabaseHelper.saveUserData(updatedUser);
        setUser(updatedUser);
        return null;
      }
